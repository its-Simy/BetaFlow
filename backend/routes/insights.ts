...
router.get('/stock/:symbol', async (req, res) => {
  try {
    ...
    const articles = await newsService.searchStockNews(
      symbol, 
      Number(lookbackDays)
    );
    ...
    const relevantArticles = relevanceService.getTopRelevantArticles(
      articles, 
      symbol, 
      Number(maxArticles)
    );
    ...
    const response = {
      ...
      metadata: {
        ...
        truncated,
        compressionSummary: contextCompressor.getCompressionSummary(
          articles.length, 
          compressed.length, 
          totalLength
        )
...
router.post('/analyze', async (req, res) => {
  try {
      ...
      for (const symbol of symbols.slice(0, 2)) { // Limit to 2 symbols
        try {
          ...
          const relevantArticles = relevanceService.getTopRelevantArticles(
            articles, 
            symbol, 
            Math.ceil(Number(maxArticles) / symbols.length)
          );
    ...
    const response = {
      ...
      sources: allArticles.length > 0 ? allArticles.slice(0, 5).map((article: any) => ({
        title: article.title,
        url: article.url,
