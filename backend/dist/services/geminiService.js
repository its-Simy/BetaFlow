...
class GeminiService {
     ...
     */
    async analyzeQuery(query, newsContext) {
        // Return mock response if API key is not set
        if (!this.apiKey || this.apiKey === 'your_gemini_api_key_here') {
     ...
     */
    buildStockAnalysisPrompt(symbol, newsContext) {
...
Return only the JSON object:`;
     ...
     */
    buildQueryAnalysisPrompt(query, newsContext) {
...
Return only the JSON object:`;
     ...
     */
    parseGeminiResponse(response, symbol) {
        try {
     ...
     */
    generateGenericKeyPoints(sentiment) {
        const genericPoints = {
            bullish: [
        ...
        return genericPoints[sentiment] || genericPoints.neutral;
    }
    /**
     * Get mock response for stock analysis (when API key is not available)
     */
    getMockStockResponse(symbol) {
        ...
        const response = mockResponses[symbol] || {
            sentiment: 'neutral',
            recommendation: 'HOLD',
            keyPoints: [
